...
# Testing encapsulated within TestLambdaHandler class.
...
class TestLambdaHandler(unittest.TestCase):
    ...
    def test_lambda_handler(self):
        ...
        # lambda function output validated against test case desired results.
        # checks for correct status code and headers i.e content type and CORs origin.
        # Throws a failure if lambda function output does not match test case assertions.
        self.assertEqual(result["statusCode"], 200)
        self.assertEqual(result["headers"]["Content-Type"], 'application/json')
        self.assertEqual(result["headers"]["Access-Control-Allow-Origin"], 'https://saifbari.com')

        # validates the incrementing by 1 feature.
        # As table is initialized to 0, any result but 1, will cause test to fail.
        self.assertEqual(result["body"], 1)


# Runs as an independent unittest script.  If script is imported as a
# module in another script, the tests are not run.
